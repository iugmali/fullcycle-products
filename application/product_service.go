...
func (s *ProductService) Create(name string, price int64) (ProductInterface, error) {
	product := NewProduct()
	product.Name = name
	product.Price = price
...
func (s *ProductService) SetPrice(product ProductInterface, price int64) (ProductInterface, error) {
	err := product.SetPrice(price)
	if err != nil {
		return &Product{}, err
	}
	result, err := s.Persistence.Save(product)
